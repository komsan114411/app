...
// ─── InstallLinkShareCard ─────────────────────────────────────
// Prominent "/install" share surface. Admin uses this to send end-users
// to a dedicated install landing page (beautiful, responsive, has
// QR, auto-detects platform, FAQ, install steps — instead of throwing
// them at the full user app).
function InstallLinkShareCard({ dl }) {
  ...
  const shareApis = (url, text) => {
    if (navigator.share) {
      navigator.share({ title: 'ดาวน์โหลดแอป', text: text || 'ติดตั้งแอปได้ที่นี่', url })
        .catch(() => {});
    } else {
      copy(url);
    }
  ...
  return (
    <Card style={{ marginTop: 14, background: 'linear-gradient(135deg, #FBFAF7, #F3EFE7)', border: '2px solid rgba(6,199,85,0.2)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        <div style={{ fontSize: 18 }}>🔗</div>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 14, fontWeight: 700 }}>หน้าแดชบอร์ดสำหรับดาวน์โหลด</div>
          <div style={{ fontSize: 11, color: '#6B6458', marginTop: 2 }}>
            ส่งลิงก์นี้ให้ผู้ใช้ → เขาเห็นหน้าเว็บสวย ๆ พร้อมปุ่มติดตั้ง + QR + คู่มือการติดตั้ง
          </div>
        </div>
      </div>
      ...
      {!ready && (
        <div style={{ padding: '10px 12px', borderRadius: 9, background: 'rgba(210,150,40,0.15)', color: '#7A5A10', fontSize: 12, marginBottom: 12 }}>
          ⚠ ยังไม่มี URL Android/iOS — ตั้งด้านบนก่อน แล้วลิงก์แดชบอร์ดถึงจะใช้งานได้
        </div>
      )}
      ...
      <div style={{
        ...
        <code style={{
          flex: 1, minWidth: 160, fontSize: 13, fontWeight: 500,
          fontFamily: 'ui-monospace, monospace',
          overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap',
          color: ready ? '#058850' : '#8F877C',
        }}>{installUrl}</code>
      </div>

      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
        <button onClick={() => copy(installUrl, 'ลิงก์แดชบอร์ด')} disabled={!ready} style={{
          padding: '9px 16px', borderRadius: 9, border: 'none',
          background: ready ? '#1F1B17' : '#8F877C', color: '#fff',
          fontFamily: 'inherit', fontSize: 13, fontWeight: 600,
          cursor: ready ? 'pointer' : 'not-allowed',
          display: 'flex', alignItems: 'center', gap: 6,
        }}>📋 คัดลอกลิงก์</button>
        <button onClick={() => window.open(installUrl, '_blank')} disabled={!ready} style={{
        ...
        <button onClick={() => setShowQr(s => !s)} disabled={!ready} style={{
          padding: '9px 14px', borderRadius: 9, border: '1px solid rgba(0,0,0,0.12)',
          background: '#fff', fontFamily: 'inherit', fontSize: 13,
          cursor: ready ? 'pointer' : 'not-allowed', opacity: ready ? 1 : 0.5,
        }}>{showQr ? 'ซ่อน QR' : '📱 แสดง QR'}</button>
        {typeof navigator !== 'undefined' && navigator.share && (
          <button onClick={() => shareApis(installUrl, 'ติดตั้งแอปได้ที่: ' + installUrl)} disabled={!ready} style={{
            padding: '9px 14px', borderRadius: 9, border: '1px solid rgba(0,0,0,0.12)',
            background: '#fff', fontFamily: 'inherit', fontSize: 13,
            cursor: ready ? 'pointer' : 'not-allowed', opacity: ready ? 1 : 0.5,
          }}>↗ แชร์</button>
      ...
      {showQr && ready && typeof QrSvg === 'function' && (
        <div style={{
          ...
          <div style={{ flex: 1, minWidth: 160 }}>
            ...
            <div style={{ fontSize: 11, color: '#6B6458', lineHeight: 1.55 }}>
              ผู้ใช้สแกน QR ด้วยกล้องมือถือ → เปิดหน้าแดชบอร์ดโดยตรง → กดปุ่มดาวน์โหลดติดตั้งได้ทันที
              <br/>บันทึกภาพ QR ได้โดยคลิกขวาที่ภาพ → Save as
            </div>
          </div>
        </div>
      )}

      <div style={{ marginTop: 12, padding: '10px 12px', borderRadius: 9, background: 'rgba(0,0,0,0.04)', fontSize: 11, color: '#6B6458', lineHeight: 1.6 }}>
        <strong>🎯 ข้อแตกต่างจากหน้าหลัก:</strong><br/>
        • หน้าแอป (<code>/</code>) — เห็นปุ่มเมนู + banner + contact<br/>
        • หน้าแดชบอร์ดติดตั้ง (<code>/install</code>) — โฟกัสที่ดาวน์โหลดอย่างเดียว พร้อม QR + คู่มือติดตั้ง + FAQ
      </div>
    </Card>
  );
}
